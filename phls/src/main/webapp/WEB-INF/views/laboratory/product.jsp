...
<c:set var="redirection" value="${contextRoot}/laboratory/product" />
<c:if test="${supplier ne null }">
<c:set var="redirection" value="${contextRoot}/laboratory/supplier-product" />
</c:if>
				...
				<table>
					...
					<c:when test="${supplier ne null}">
						<tr>
							...
							<td><input type="text"  class="form-control" value="${supplier.nameOfEstablishment }" disabled="true"/>
					...
					<c:when test="${edit}">
						<tr>
							<td><label>Company</label></td>
							<td><input type="text"  class="form-control" value="${product.supplier.nameOfEstablishment }" disabled="true"/>
					...
					<tr>
						...
						<td>
							<c:if test="${edit}">
								<sf:input path="code" class="form-control" placeholder="Product Code" required="required" disabled="true" />
							</c:if>
							<c:if test="${not edit}">
					...
					<tr>
						...
						<td>
							<c:if test="${edit}">
								<sf:input path="name" class="form-control" placeholder="Product Name" required="required" disabled="true"/>
							</c:if>
							<c:if test="${not edit}">
...
<%-- 					<tr>
					...
					<tr>
						...
						<sf:input type="hidden" path="quantity" />
						<sf:input type="hidden" path="unitPrice" />
						<td>
							<c:if test="${supplier ne null}">
